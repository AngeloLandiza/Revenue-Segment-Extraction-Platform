...
class ExtractionSettings:
    ...
    def from_env(cls) -> "ExtractionSettings":
        return cls(
            provider_name=os.getenv("RSE_EXTRACTION_PROVIDER", DEFAULT_EXTRACTION_PROVIDER),
            model=os.getenv("RSE_EXTRACTION_MODEL", DEFAULT_EXTRACTION_MODEL),
            verification_model=os.getenv(
                "RSE_VERIFICATION_MODEL",
                os.getenv("RSE_EXTRACTION_MODEL", DEFAULT_EXTRACTION_MODEL),
            ),
            arbitration_model=os.getenv(
                "RSE_ARBITRATION_MODEL",
                os.getenv("RSE_EXTRACTION_MODEL", DEFAULT_ARBITRATION_MODEL),
            ),
            page_bundle_size=_int_from_env(
                "RSE_EXTRACTION_PAGE_BUNDLE_SIZE",
                DEFAULT_PAGE_BUNDLE_SIZE,
            ),
            max_tokens=_int_from_env("RSE_EXTRACTION_MAX_TOKENS", DEFAULT_MAX_TOKENS),
            temperature=_float_from_env("RSE_EXTRACTION_TEMPERATURE", DEFAULT_TEMPERATURE),
            enable_second_pass_verification=_bool_from_env(
                "RSE_ENABLE_SECOND_PASS_VERIFICATION",
                DEFAULT_ENABLE_SECOND_PASS_VERIFICATION,
            ),
            enable_arbitration=_bool_from_env(
                "RSE_ENABLE_ARBITRATION",
